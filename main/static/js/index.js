...
function bus_search() {
    if(!document.querySelector("#bus-from").dataset.value) {
        alert("Please select bus origin.");
        return false;
    }
    if(!document.querySelector("#bus-to").dataset.value) {
        alert("Please select bus destination.");
        return false;
    }
...
var app = new Vue({
    ...
    mounted() {
      axios.get('/current_time/')
        .then(response => {
          this.currentTime = response.data.current_time;
          setInterval(() => {
            this.currentTime = (new Date(Date.parse(this.currentTime) + 1000)).toLocaleTimeString();
          }, 1000);
        })
        .catch(error => {
          console.log(error);
